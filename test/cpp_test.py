...
from source.cpp.solution import cppSolution  # 🐍 Import the C++ wrapper
...
class test_cpp_Solution(unittest.TestCase):
    ...
    # ⏱️ Run each test with a timeout to avoid infinite loops
    @timeout(0.5)
    def test(self):
        # 🔁 Loop through all test cases
        for nums, expected in self.__testcases:
            # 🔹 subTest allows individual cases to fail without stopping the loop
            with self.subTest(nums):
                # 🧮 Call the C++ h-index function
                actual = self.__solution.cpp_hIndex(nums)
